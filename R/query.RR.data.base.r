...
query.RR.data.base <- function(dart_data, basedir, species, dataset, RR_db_file, overwrite_meta=TRUE) {
   ...
   if (writing) {
      ...
      i_all_tissues <- match(row.names(dart_data$gt), all_tissues$tissueSampleIRN)
      if( length(  which(is.na(i_all_tissues)) ) > 0 ) {
       ...
       meta_data_table <- all_tissues[i_all_tissues,c("tissueSampleIRN", "siteID", "tissueSampleLatitude", "tissueSampleLongitude")]
       colnames(meta_data_table) <- c("sample", "site", "lat", "long")
