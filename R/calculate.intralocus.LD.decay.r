...
calculate.intralocus.LD.decay <- function(dart_data, MAF_thresh=0.2, min_dist_thresh_bp=10) {

   # load some required libraries
   require(genetics)
   require(LDcorSV)

   pairs <- get.intra.locus.pairs(dart_data, MAF_thresh, min_dist_thresh_bp) 
   npairs <- nrow(pairs)
